...
class KuramanimeProvider : MainAPI() {
    override var mainUrl = "https://v6.kuramanime.run"
    ...
    private fun getProperAnimeLink(uri: String): String {
        return if (uri.contains("/episode")) {
            uri.substringBeforeLast("/episode/") + "/"
        } else {
            uri
        }
    }
    ...
    private fun Element.toSearchResult(): AnimeSearchResponse? {
        ...
        val posterUrl = fixUrl(this.selectFirst("div.product__item__pic.set-bg")?.attr("data-setbg") ?: "")
        val episode = this.selectFirst("div.ep span")?.text()?.let {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val title = document.selectFirst(".anime__details__title > h3")?.text()?.trim() ?: return ErrorResponseException("No title found").throwIt()
        val poster = fixUrlNull(document.selectFirst(".anime__details__pic")?.attr("data-setbg"))
        val tags = document.select("div.anime__details__widget > div > div:nth-child(2) > ul > li:nth-child(1)")
            .text().trim().removePrefix("Genre: ").split(", ")

        val year = document.select("div.anime__details__widget > div > div:nth-child(1) > ul > li:nth-child(5)")
            .text().trim().removePrefix("Musim: ")
            .filter { it.isDigit() }
            .toIntOrNull()
            
        val status = getStatus(
            document.select("div.anime__details__widget > div > div:nth-child(1) > ul > li:nth-child(3)")
                .text().trim().removePrefix("Status: ")
        )
        val description = document.selectFirst(".anime__details__text > p")?.text()?.trim()

        val episodes = mutableListOf<Episode>()
        var page = 1
        while (true) {
            val doc = app.get("$url?page=$page").document
            val eps = Jsoup.parse(doc.selectFirst("#episodeLists")?.attr("data-content") ?: break)
        ...
        val recommendations = document.select("div#randomList > a").mapNotNull {
            ...
            val epTitle = it.selectFirst("h5.sidebar-title-h5.px-2.py-2")?.text() ?: return@mapNotNull null
            val epPoster = fixUrlNull(it.selectFirst(".product__sidebar__view__item.set-bg")?.attr("data-setbg"))
        ...
        return newAnimeLoadResponse(title, url, type) {
            ...
            tracker?.malId?.let { addMalId(it) }
            tracker?.aniId?.toIntOrNull()?.let { addAniListId(it) }
        }
    }
